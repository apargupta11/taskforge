...
const KanbanBoard = () => {
  ...
  const canEditTask = (task) => isAdmin || (user?.role === 'member' && task.assigned_to === user.id);
  const canCreateTask = isAdmin || user?.role === 'member';
            ...
            {['todo', 'in-progress', 'done'].map(status => (
              <TaskColumn 
                ...
                onDrop={id => {
                  ...
                  if (task && canEditTask(task)) handleUpdateTaskStatus(id, status);
                }}
              >
                    ...
                    <td className="py-3 px-6 text-right">
                      {canEditTask(task) && (
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => setEditTask({...task})} className="text-slate-500 hover:text-violet-400"><Edit3 className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleDeleteTask(task.id)} className="text-slate-500 hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      )}
                    </td>
                  </tr>
